...
class ApiPermission(BasePermission):
    ...
    def _has_permission(self, view, obj, request):
        ...
        if request.auth:
            if event:
                if hasattr(request.auth, 'events'):
                    if event not in request.auth.events.all():
                        return False
                elif hasattr(request.auth, 'has_event_permission'):
                    organizer = getattr(request, 'organizer', event.organizer)
                    if not request.auth.has_event_permission(organizer, event, request=request):
                        return False
                else:
                    return False
