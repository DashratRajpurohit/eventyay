...
def test_log_out_after_absolute_timeout(user_client, team, organizer, event):
    ...
    session['eventyay_auth_long_session'] = False
    session['eventyay_auth_login_time'] = int(time.time()) - 3600 * 12 - 60
    session.save()
...
def test_log_out_after_relative_timeout(user_client, team, organizer, event):
    session = user_client.session
    session['eventyay_auth_long_session'] = False
    session['eventyay_auth_login_time'] = int(time.time()) - 3600 * 6
    session['eventyay_auth_last_used'] = int(time.time()) - 3600 * 3 - 60
    session.save()
...
def test_dont_logout_by_relative_in_long_session(user_client, team, organizer, event):
    session = user_client.session
    session['eventyay_auth_long_session'] = True
    session['eventyay_auth_login_time'] = int(time.time()) - 3600 * 5
    session['eventyay_auth_last_used'] = int(time.time()) - 3600 * 3 - 60
    session.save()
...
def test_update_session_activity(user_client, team, organizer, event):
    ...
    session['eventyay_auth_long_session'] = False
    session['eventyay_auth_login_time'] = int(time.time()) - 3600 * 5
    session['eventyay_auth_last_used'] = t1
    session.save()

    response = user_client.get('/api/v1/organizers/{}/events/'.format(organizer.slug))
    assert response.status_code == 200

    assert user_client.session['eventyay_auth_last_used'] > t1
